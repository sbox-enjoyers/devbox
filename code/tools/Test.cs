...
[Library( "test_rope", Title = "test", Description = "Join two things together with a rope", Group = "construction" )]
[Tool( "tools", "constraints" )]
public partial class TestTool : BaseTool
{
	...
	public override void Simulate()
	{
		...
		using ( Prediction.Off() )
		{
			...
			var spring = PhysicsJoint.CreateSlider( targetBody, tr.Body, targetBody.Rotation.Forward, 0, 300 );

			DebugOverlay.Box( tr.Body.Position.ToScreen(), tr.Body.Rotation, tr.Body.GetBounds().Mins, tr.Body.GetBounds().Maxs, Color.Green, 10 );
			//DebugOverlay.Box( tr.Body.Transform.Position, tr.Body.Rotation, tr.Body.GetBounds().Mins, tr.Body.GetBounds().Maxs, Color.Red, 10 );
